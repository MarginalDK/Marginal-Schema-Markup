...
/*
 * Undgå dobbelt-indlæsning, fx hvis pluginnet ligger i to mapper (eller både som plugin og
 * mu-plugin). Den første kopi kører; den anden viser kun en besked til administratoren.
 */
if ( defined( 'MARGINAL_SCHEMA_VERSION' ) ) {
	if ( function_exists( 'marginal_schema_bootstrap_notice' ) ) {
		marginal_schema_bootstrap_notice(
			sprintf( 'Pluginnet er installeret mere end én gang. Kopien i mappen "%s" bruges ikke – deaktivér og slet den under Plugins.', basename( __DIR__ ) )
		);
	}
	return;
}

define( 'MARGINAL_SCHEMA_VERSION', '1.0.1' );
...
/*
 * VIGTIGT: Funktionerne skal stå inde i denne if-blok. Funktioner på øverste niveau "hejses"
 * af PHP og erklæres, før koden ovenfor kører – en anden kopi af filen ville så give den
 * fatale fejl "Cannot redeclare function" i stedet for at stoppe pænt ved return.
 */
if ( ! function_exists( 'marginal_schema_bootstrap' ) ) {
